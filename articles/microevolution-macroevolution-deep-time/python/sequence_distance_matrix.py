"""
Sequence distance matrix with Jukes-Cantor correction.
...
def pairwise_distance(seq1: str, seq2: str) -> tuple[int, float, float]:
    """Return mismatches, p-distance, and Jukes-Cantor distance."""
    ...
    if p_distance >= 0.75:
        ...
        jukes_cantor = -(3 / 4) * np.log(1 - (4 / 3) * p_distance)

    return mismatches, p_distance, jukes_cantor


def main() -> None:
    """Build pairwise divergence table and symmetric distance matrix."""

    seq_df = pd.read_csv(SEQUENCES_PATH)
    seqs = dict(zip(seq_df["lineage"], seq_df["sequence"]))
